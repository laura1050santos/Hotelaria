...
function atualizarDados() {
    ...
    console.log("Usuários após a filtragem:", usuarios);
    ...
    console.log("Usuário logado atualizado:", usuarioLogado);
    ...
    usuarios.push(novoUsuario);
    console.log("Usuários após adicionar o novo usuário:", usuarios);
...
function excluirCadastroCliente(){
  ...
  const  hospedeSelecionado =document.getElementById("selecionarHospedeExcluir")//recebe o cpf do usuario selecionado
  ...
  return;
}else{ return alert("Hospede não encontrado")}
}
// ========== Funções de Reservas ==========
...
carregarUsuarios()
    const selectElement = document.getElementById("selecionarHospede");
    usuarios.forEach(cliente => {
        const novaOpcao = document.createElement("option");
        novaOpcao.value = cliente.cpf;
...
function preencherHospedesAtt(){
    carregarUsuarios()
        const selectElement = document.getElementById("selecionarHospedeAtt");
        usuarios.forEach(cliente => {
            const novaOpcao = document.createElement("option");
            novaOpcao.value = cliente.cpf;
            novaOpcao.textContent = cliente.nome;
...
function preencherHospedesExcluir(){
    carregarUsuarios()
    const selectElement = document.getElementById("selecionarHospedeExcluir");
    usuarios.forEach(cliente => {
        const novaOpcao = document.createElement("option");
        novaOpcao.value = cliente.cpf;
        novaOpcao.textContent = cliente.nome;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const atualizarDadosFormCliente = document.getElementById("atualizarDadosFormCliente");
...
const ExcluirClienteForm = document.getElementById("selecionarHospedeExcluir")
if(ExcluirClienteForm){
    ExcluirClienteForm.addEventListener("submit",event=>{
        event.preventDefault();
